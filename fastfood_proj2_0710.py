...
with st.sidebar:
    st.markdown("Author: **:blue[Group 3]**")
    st.write("Date: ", datetime.date(2023, 7, 10))
...
tabs = st.tabs(["Box plot", "Histogram"])
...
with tabs[0]:
        _df = df[['item', 'calories', 'restaurant']].sort_values('calories', ascending=False).iloc[:10]
        fig5 = px.bar(_df, y='item', x='calories', text_auto=True, labels=_labels, color='restaurant', 
                     title='Top 10 meals with highest calories')
        fig5.update_layout(yaxis = {"categoryorder":"total ascending"})
        show(fig5)

with tabs[1]:
        _df = df[['item', 'calories', 'restaurant']].sort_values('calories', ascending=True).iloc[:10]
        fig5 = px.bar(_df, y='item', x='calories', text_auto=True, labels=_labels, color='restaurant', 
